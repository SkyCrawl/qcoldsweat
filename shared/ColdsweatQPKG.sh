...
# disabling the QPKG though - there seems to be no truncating when starting it...
# Weird, QNAP... weird. As such, successful launch of Coldsweat depends on previous
# successful launch of Entware-ng. That's why the RC_NUMBER is set to 102 (Entware-ng
# is 101).
CMD_PYTHON="/opt/bin/python"

# Note: let's take advantage of the installed grep.
CMD_GREP="/usr/local/bin/grep"
